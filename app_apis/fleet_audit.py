...
def _erp_fleet() -> tuple[dict, list, dict]:
	...
	# Two facts hang off the same Serial No, so they are read in one pass:
	#
	#   carrier  item_code -> Item.item_name ("Lebara SIM", "STC SIM", ...)
	#   plan     custom_sim_type -> SIM Type ("Normal", "Data 30G", ...) --
	#            the field that carries a price
	#
	# `custom_sim_type` is a Custom Field, so it is not guaranteed to exist on
	# a fresh install of this app. Checked, not assumed, the same way
	# `has("sim_serial")` guards the join itself.
	sim_items: dict[str, str] = {}
	sim_types: dict[str, str] = {}
	if has("sim_serial"):
		plan_col = (
			"sn.`%s`" % SIM_TYPE_FIELD
			if frappe.db.has_column("Serial No", SIM_TYPE_FIELD)
			else "null"
		)
		# LEFT JOIN, not JOIN: a Serial No can carry a plan without an
		# item_code, and an inner join would drop that priced SIM entirely.
		for name, item_name, plan in frappe.db.sql(
			"""
			select sn.name, it.item_name, %s
	...
	for r in rows:
		...
		r["_sim_type"] = sim_types.get(r.get("sim_serial")) if r.get("sim_serial") else None
...
def _build_rows(erp_by_imei, erp_without, im_by_imei, pilot_by_imei, audited_at,
	...
	for imei in keys:
		...
		out.append({
			...
			# The SIM's plan -- Serial No.custom_sim_type -> SIM Type, which is
			# where the price lives. The plan is stored; the price is NOT, so a
			# corrected tariff takes effect without re-running the audit.
			"sim_type": (erp or {}).get("_sim_type"),
...
COLUMNS = (
	...
	"sim_item", "sim_type", "sim_status", "sim_last_seen", "sim_msisdn",
...
# Arabic as it is typed is not Arabic as it is stored, and the Vehicle and
# Customer columns are almost entirely Arabic. Two habits break a plain LIKE:
#
#   spacing   a plate keeps its letters apart -- "9880 - أ د ص" -- and nobody
#             searching for that plate types the spaces or the dash. 21,059 of
#             the 22,437 plates carry at least one space.
#   spelling  whichever alef, hamza or teh marbuta the person entering the row
#             reached for is the only one that will ever match it. Plates hold
#             both أ (16,595) and ا (1,708); customer names hold 39,469 ة.
#
# Digits have neither problem, which is why searching a plate by its numbers
# always worked and searching it by its letters practically never did. Folding
# BOTH the stored value and the typed term to the same skeleton before
# comparing them fixes that, and only ever widens what matches.
# Written as escapes, not as the letters themselves: half of these are
# invisible or indistinguishable in a diff, and an editor that helpfully
# "fixes" a stray hamza would silently break the fold.
_FOLD = (
	...
	("\u0629", "\u0647"),   # teh marbuta            ة -> ه
	("\u0621", ""),         # standalone hamza       ء -> dropped
	("\u0640", ""),         # tatweel                ـ -> dropped
	("\u00a0", ""),         # non-breaking space, common in pasted text
	(" ", ""),            # ordinary space -- the one that hides every plate
	("-", ""),            # and the dash between a plate's halves
)


def _fold(term: str) -> str:
	"""The typed side of the comparison."""
	out = str(term or "")
	for a, b in _FOLD:
		out = out.replace(a, b)
	return out
...
def _fold_sql(col: str) -> str:
	"""The stored side -- the same folding, expressed over a column.

	Nested REPLACE means no index can serve these filters. That is affordable,
	and it was already true: a leading-wildcard LIKE never used one either. The
	snapshot is one flat table of ~25,000 rows, and both the page query and its
	COUNT scan it in a few milliseconds.
	"""
	expr = "ifnull(%s, '')" % col
	for a, b in _FOLD:
		expr = "replace(%s, %s, %s)" % (expr, frappe.db.escape(a), frappe.db.escape(b))
	return expr
...
def get_rows(verdict: str = "", erp_status: str = "", on_pilot: str = "", on_im: str = "",
             ...
             sim_msisdn: str = "", no_sim: str = "", sim_item: str = "", sim_type: str = "",
	...
	# Both sides folded -- see _FOLD above. أدص now finds "9880 - أ د ص", and
	# which alef or teh marbuta a name happens to be spelled with stops
	# deciding whether it can be searched at all.
	if vehicle:
		where.append("(%s like %%s or %s like %%s)" % (_fold_sql("plate"), _fold_sql("erp_vehicle")))
		vals += [like(_fold(vehicle)), like(_fold(vehicle))]
	if customer:
		where.append("(%s like %%s or %s like %%s)" % (_fold_sql("customer_name"), _fold_sql("customer")))
		vals += [like(_fold(customer)), like(_fold(customer))]
	if sim_status:
	...
	if sim_type:
		# UNSET is offered as a real choice: "which SIMs have no plan set yet"
		# is the question that clears the unpriced count off the tiles.
		if sim_type == UNSET:
			where.append("ifnull(sim_type, '') = ''")
		else:
			where.append("ifnull(sim_type, '') = %s")
			vals.append(sim_type)
	...
	if search:
		# Folded too: this box reaches the same two Arabic columns, so without
		# it it would keep the exact bug the Vehicle and Customer filters had.
		cols = ("imei", "plate", "erp_vehicle", "customer_name", "pilot_vehicle", "im_vehicle")
		where.append("(" + " or ".join("%s like %%s" % _fold_sql(c) for c in cols) + ")")
		vals += [like(_fold(search))] * len(cols)
	...
	# Priced here rather than in SQL: one dict lookup per row against a table
	# of four tariffs, versus a join on every page load for a value that never
...
# The Vehicle and Customer filters search text, so "" there means "no filter".
# The plan filter cannot work that way: "no plan set" is a real answer somebody
# needs to ask for, so it travels as a value that is not the empty string.
UNSET = "(none)"


def _has_untyped() -> bool:
	return bool(frappe.db.sql(
		"select 1 from `tab%s` where ifnull(sim_type, '') = '' limit 1" % SNAPSHOT_DT
	))
...
def get_filter_options() -> dict:
	...
	return {
		...
		"sim_types": distinct("sim_type") + ([UNSET] if _has_untyped() else []),
	}
